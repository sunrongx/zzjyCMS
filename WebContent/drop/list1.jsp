...
<!-- 	<div class="box-positon">
		...
		<form class="ropt">
			<input class="add" type="button" value="添加"
				onclick="javascript:window.location.href='dept.do'" />
		</form>
		<div class="clear"></div>
	</div>
 -->	
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${users }" var="user">
						<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'"
							...
							<a href="userdelete.do?id=${ user.id } " onclick="if(!confirm('确定删除吗？')){return false; }" class="pn-opt">删除</a></td>
						</tr>
					</c:forEach>
...
<%-- 	<tr>
			<td align="right" colspan="10">
				...
				<a href="userlist.do?page=${page==1?1:page-1 }">上一页</a>
				<a href="userlist.do?page=${page==pageCount?pageCount:page+1 }">下一页</a>
				<a href="userlist.do?page=${pageCount }">尾页</a>
			</td>
		</tr>
 --%>
			...
			<div class="page pb15"  >
				...
				<input style="background-color:white;border-radius:2px;border:solid 3px lightblue "  type="submit" value="批量删除" onclick="if(!confirm('确定删除吗？')){return false; }" />
			</div>
			<div class="page pb15" style="float: right;">
				<span class="r inb_a page_b"> <!-- [当前页/尾页] -->
					[${requestScope.currentPage }/${requestScope.pageCount }] <a
					href="userlist.do?currentPage=1&&name=${name }">首页</a> <c:if
						test="${requestScope.currentPage-1>0 }">
						<a
							href="userlist.do?currentPage=${requestScope.currentPage-1 }&&name=${requestScope.name}">上一页</a>
					</c:if> <c:if
						test="${requestScope.currentPage+1<=requestScope.pageCount }">
						<a
							href="userlist.do?currentPage=${requestScope.currentPage+1 }&&name=${requestScope.name}">下一页</a>
					</c:if> <a
					href="userlist.do?currentPage=${requestScope.pageCount }&&name=${requestScope.name}">尾页</a>
				</span>
			</div>
			<div class="page pb15"  style="float:right;margin:-15px 30px 0 0" >
				<form action="userlist.do?currentPage=${requestScope.currentPage}" >
					转到第<input type="text" width="10px" name="currentPage"/>
					页<input type="submit" value="确定"/>
				</form>
			</div>
